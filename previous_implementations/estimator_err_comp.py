...
def h_rss_model(x_pos_mobil, h_mobil, x_pos_stat, h_stat, lambda_ti, gamma_ti, theta_cap, psi_low, theta_low, n_tx, n_rec, d_r, d_t):
    ...
    # rss1 = -20 * np.log10(r) + r * lambda_ti + gamma_ti + np.log10((np.cos(psi_low) ** 2) * d_r * (np.cos(theta_cap) ** n_tx) * d_t * (np.cos(theta_cap + theta_low) ** n_rec))  # TODO: Why not this one ?
